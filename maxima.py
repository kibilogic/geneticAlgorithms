...
# examine GA operations using sin(x) - 0.2 * abs(x) to find the maxima 
# individuals in the population will try to climb as high as possible
...
def mutate_gaussian(g, mu, signma, min=None, max=None):

	mutated_gene = g+ random.gauss(mu,signma)
...
while generation_number < MAX_GENERATIONS:
	...
	for mutant in crossed_offspring:
		...
		if random.random() < MUTATION_PROBABILITY:
			...
			mutatated_offspring.append(new_mutant)
		else:
			mutatated_offspring.append(mutant)


		population = mutatated_offspring.copy()		
